    ...
    try {
        ...
        if (mysqli_num_rows($result1) > 0) {
            while ($rowss = mysqli_fetch_assoc($result1)) {
                ...
                $sql = "UPDATE add_property SET city ='$city' && ward_no='$ward_no' && area='$area' && contact_no = '$contact_no' && property_type = '$property_type' && estimated_price = '$estimated_price' && total_rooms='$total_rooms' && bedroom ='$bedroom' && living_room ='$living_room' && kitchen='$kitchen' && description = '$description' && latitude='$latitude'&& longitude='$longitude' && owner_id='$owner_id' && booked='$booked'&& street_No='$street_No') VALUES ('$city', '$ward_no', '$area', '$contact_no', '$property_type', '$estimated_price', '$total_rooms', '$bedroom', '$living_room', '$kitchen', '$description', '$latitude', '$longitude', '$owner_id', '$booked', '$street_No')";
                $query = mysqli_query($db, $sql);
                if ($query) {
                    $property_id = mysqli_insert_id($db);
                    $countfiles = count($_FILES['p_photo']['name']);
